...
import re, pickle, os, requests, base64, difflib, json, io, numpy as np
...
def check_url(url):
    ...
    if len(parsed.netloc.split(".")) > 4:
        score += 10
        reasons.append({"icon": "🔗", "text": "Excessive subdomains detected"})
    ...
    try:
        ...
        if whois_data and whois_data["new_domain"]:
            score += 20
            reasons.append({"icon": "📅", "text": f"Brand new domain — only {whois_data['age_days']} days old"})
    except Exception as e:
        print(f"WHOIS error: {e}")
    ...
    try:
        ...
        if typo_matches:
            top = typo_matches[0]
            score += 25
            reasons.append({"icon": "🎯", "text": f"Typosquatting: looks like '{top['brand']}' ({top['score']}% similar)"})
    except Exception as e:
...
def generate_pdf_report(url, data):
    ...
    title_style   = ParagraphStyle("title",   parent=styles["Title"],   fontSize=22, spaceAfter=4,  textColor=colors.HexColor("#7c5cfc"))
    sub_style     = ParagraphStyle("sub",     parent=styles["Normal"],  fontSize=10, spaceAfter=16, textColor=colors.HexColor("#888888"))
    h2_style      = ParagraphStyle("h2",      parent=styles["Heading2"],fontSize=13, spaceBefore=14,spaceAfter=6, textColor=colors.HexColor("#333333"))
    body_style    = ParagraphStyle("body",    parent=styles["Normal"],  fontSize=10, leading=16,    textColor=colors.HexColor("#222222"))

    verdict_colors = {"phishing": colors.HexColor("#ff4060"), "suspicious": colors.HexColor("#ffb344"),
                      "low": colors.HexColor("#ffb344"), "safe": colors.HexColor("#00e096")}
    ...
    verdict_text  = {"phishing":"PHISHING DETECTED","suspicious":"SUSPICIOUS URL","low":"LOW THREAT","safe":"URL IS SAFE"}.get(data["status"],"UNKNOWN")
    verdict_style = ParagraphStyle("verdict", parent=styles["Normal"], fontSize=16, fontName="Helvetica-Bold", textColor=vc, spaceAfter=12)
    ...
    if vt and not vt.get("scanning"):
        ...
        vt_rows = [
            ...
            ["Total",      str(vt.get("total",0))],
        ]
        if vt.get("flagged_by"):
            vt_rows.append(["Flagged By", ", ".join(vt["flagged_by"])])
        if vt.get("categories"):
            vt_rows.append(["Categories", ", ".join(vt["categories"])])
        vt_table = Table(vt_rows, colWidths=[50*mm, 125*mm])
    ...
    story.append(Paragraph(
        f"Generated by TrapNet · {datetime.utcnow().strftime('%d %b %Y %H:%M UTC')}",
...
def bulk_scan():
    ...
    urls = [u.strip() for u in body.get("urls", []) if u.strip()][:20]
    if not urls:
...
def get_history():
    return jsonify({"history": scan_history[:50], "total": len(scan_history)})
